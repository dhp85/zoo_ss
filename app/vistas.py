...
class VistaGrupo:
    ...
    def paint(self):
        ...
        for indice, tipo in enumerate(TipoEntrada):
            locate(self.x, self.y + 3 + indice, f"{tipo.name:.<14s}{tipo.value.precio:5.2f}    {self.grupo.cantidad_entradas_tipo(tipo):2d}     {self.grupo.subtotal_tipo(tipo):7.2f}")
...
class VistaInputEntero(VistaInput):
    def paint(self):
        while True:
            locate(self.x, self.y, self.etiqueta)
            cadena = super(). paint()
